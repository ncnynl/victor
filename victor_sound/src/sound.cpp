...
#include <victor_sound/SayAction.h>
#include <victor_sound/PlayAction.h>
#include <victor_srvs/SetString.h>
...
protected:
  ...
  ros::NodeHandle nh_;
  actionlib::SimpleActionServer<victor_sound::SayAction> as_say_;
  actionlib::SimpleActionServer<victor_sound::PlayAction> as_play_;
...
public:
  ...
  void as_goal_cb_play_()
  {
    ...
    {
      victor_sound::PlayResult result;
  ...
  void as_cb_say_(const victor_sound::SayGoalConstPtr &goal)
  {
    std::string message;
    bool ret = say(goal->text, message);
    
    victor_sound::SayResult result;
    result.success = ret;
    result.message = message;
    if (ret)
    {
  ...
  bool service_cb_say(victor_srvs::SetString::Request &req,
                      victor_srvs::SetString::Response &res )
  {
    res.success = say(req.data, res.message);
    return true;
  }

  bool service_cb_play(victor_srvs::SetString::Request &req,
                       victor_srvs::SetString::Response &res )
  {
  ...
  void timer_play_feedback_cb(const ros::TimerEvent&)
  {
    if(as_play_.isActive())
    {
      if (libvlc_media_player_is_playing(vlc_player_) == 1)
      {
        ...
        victor_sound::PlayFeedback feedback;
